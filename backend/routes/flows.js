...
router.get('/:id', auth, (req, res) => {
  const row = db.prepare('SELECT * FROM flows WHERE id=? AND user_id=?').get(req.params.id, req.user.id);
...
router.post('/:id/clone', auth, (req, res) => {
  const src = db.prepare('SELECT * FROM flows WHERE id=? AND user_id=?').get(req.params.id, req.user.id);
  if (!src) return res.status(404).json({ error: 'Not found' });
  const id = uuidv4();
  db.prepare('INSERT INTO flows (id,user_id,name,nodes,edges) VALUES (?,?,?,?,?)')
    .run(id, req.user.id, src.name + ' (Copy)', src.nodes, src.edges);
